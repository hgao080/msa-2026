...
- "Let's go with B"
- "The Board page is a bit iffy on mobile. The age and updated text doesn't sit nicely flush against the right... It would also be nice to move the log application button to be at the bottom right as a floating buttton for ease of access"
- "Make the log button a smaller circle on mobile and I don't think its nice that the company and role text gets truncated as the screen shrinks. I think it makes more sense for source and pipeline to disappear off first"
- "Verify and test with agent-browser"
- "Run docker build -t $ACR_LOGIN_SERVER/horme-api:... backend/Horme.API" (pasted a failed build log) — "Why did my CD not work?"
...
- User caught two follow-up mobile bugs from a screenshot: the Age/Updated column wasn't flush right on the board, and asked for the "Log application" button as a floating bottom-right button. Root cause of the misalignment: `ApplicationRow`'s mobile grid auto-placed the status badge and age text as two *separate* items once Source/Pipeline were hidden, so they could land in different columns/rows depending on how many siblings were hidden. Fixed by wrapping both in one `<span>` that's `display: contents` on desktop (invisible to the grid, children keep their original column slots) and `flex flex-col items-end` only below 640px (collapses to a single flush-right grid item). Reused this exact wrapper technique rather than hand-tuning column widths per breakpoint.
- Added a `fixed`-position circular FAB for `AddApplication` on mobile (inline button hidden via `max-[640px]:hidden`), but it first rendered pinned to the top-right instead of the bottom — the same `animate-launch` ancestor-transform bug noted in `[[design-direction]]` for `Modal.tsx` (a `transform` on an ancestor creates a new containing block for `position: fixed` descendants). Fixed the same way: `createPortal` to `document.body`.
- User flagged that Company/Role was truncating before Source/Pipeline dropped off as the viewport narrowed, and wanted the drop order reversed. Moved Source/Pipeline's hide breakpoint from 640px to (initially) 900px so Company/Role gets the freed column width first; also shrank the FAB from 56px to 44px per request.
- Re-verified with `agent-browser` (installed + used per the new `frontend/CLAUDE.md` guidance, replacing the raw Playwright scripts from earlier in this session) instead of assuming the diff was correct. Direct `getComputedStyle` checks at the literal boundary pixel caught a real bug: Tailwind v4's `max-[Npx]` variant compiles to `@media not (min-width: Npx)`, which is *exclusive* — at exactly `900px` the Source/Pipeline columns were still `display: block`, not hidden. Bumped both call sites to `max-[901px]` and reconfirmed the boundary hides correctly at exactly 900px.
- Separately, user pasted a failed `docker build ... backend/Horme.API` log (`COPY --from=build /app/publish .` → `"/app/publish": not found`) while pushing an image tagged off this session's commit, asking why CD didn't work. Traced it to `backend/Horme.API/Dockerfile`: Session 18's `FROM build AS dev` stage was inserted *before* the existing `COPY . .` / `RUN dotnet publish` lines, so those two lines silently became part of the `dev` stage instead of `build`. A plain `docker build` (no `--target`, defaults to the last stage, `final`) never builds `dev` — `final` only depends on `build` via `COPY --from=build` — so `build` never got past `dotnet restore` and `/app/publish` never existed. Fixed by moving `COPY . .` / `dotnet publish` back above the `dev` stage split; verified with a local `docker build` (full 6-step `build` stage, publish + final COPY both succeed), then removed the local test image.
- Mid-session, noticed the repo's checked-out branch had changed from `dev` (session start) to `main` (by the time of the Docker fix commit) with no `git checkout` run in this session. Flagged it to the user; left unresolved/unaddressed as of session end — not something this session did.
